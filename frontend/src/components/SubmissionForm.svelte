<script>
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      console.error(err);
      errorMessage =
        err.message || "Connection to server failed. Please try again.";
    } finally {
      loading = false;
    }
  }
...
<div class="w-full h-full flex flex-col justify-center">
  {#if success}
    <div
      class="flex flex-col items-center justify-center text-center py-10 px-4 transition-all duration-300"
    >
      <!-- TCG Card Game themed success illustration -->
      <div
        class="relative w-28 h-40 bg-amber-100 border-4 border-black rounded-2xl flex flex-col items-center justify-center rotate-6 shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] mb-8"
      >
        <!-- Inside Card Cost Badge -->
        <div
          class="absolute -top-3 -right-3 w-8 h-8 rotate-45 border-2 border-black bg-white flex items-center justify-center shadow-sm"
        >
          <span
            class="-rotate-45 block font-serif font-black text-sm text-purple-950"
            >✓</span
          >
        </div>
        <!-- Card Art Frame Mockup -->
        <div
          class="w-[85%] h-20 bg-purple-900 border-2 border-black rounded-lg flex items-center justify-center mb-2 overflow-hidden"
        >
          <span class="text-white text-3xl">✨</span>
        </div>
        <!-- Monospace text footer -->
        <div
          class="text-[8px] font-mono tracking-widest text-gray-800 uppercase mt-2"
        >
          SUCCESS
        </div>
      </div>
      ...
      <div
        class="bg-[#F1E8D9] border-2 border-[#3A2E1A]/20 shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] rounded-xl p-6 max-w-xs sm:max-w-sm mx-auto text-center mb-8"
      >
  ...
  {:else}
    <form on:submit={handleSubmit} class="space-y-4">
      {#if errorMessage}
        <div
          class="p-3 bg-red-50 border-2 border-red-500 text-red-700 text-xs rounded-lg font-sans"
        >
          <strong>Error:</strong>
          {errorMessage}
        </div>
      ...
      {#if currentForm === "tip"}
        <div>
          <label
            for="tip-name"
            class="block text-xs font-bold uppercase text-purple-950 mb-1"
            >Your Name / Handle *</label
          >
          <input
            id="tip-name"
            type="text"
        ...
        {#if currentForm === "request"}
          ...
          <div>
            <label
              for="req-details"
              class="block text-xs font-bold uppercase text-purple-950 mb-1"
              >Request Details / Topic Description *</label
            >
            <textarea
              id="req-details"
        ...
        {:else}
          ...
          <div>
            <label
              for="moment-details"
              class="block text-xs font-bold uppercase text-purple-950 mb-1"
              >Milestone Details & Event Description *</label
            >
            <textarea
              id="moment-details"
